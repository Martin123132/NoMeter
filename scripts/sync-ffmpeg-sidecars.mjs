...
const ffmpegRoot = envValue('NOMETER_FFMPEG_ROOT', 'OPENFORGE_FFMPEG_ROOT') || 'D:\\Codex\\OpenForge\\tools\\ffmpeg'
...
const ffmpeg = findFile(ffmpegRoot, 'ffmpeg.exe')
const ffprobe = findFile(ffmpegRoot, 'ffprobe.exe')

if (!ffmpeg || !ffprobe) {
  console.error(`Could not find ffmpeg.exe and ffprobe.exe under ${ffmpegRoot}`)
  process.exit(1)
}

mkdirSync(binariesDir, { recursive: true })
copySidecar(ffmpeg, 'ffmpeg', triple)
copySidecar(ffprobe, 'ffprobe', triple)
