...
- [ ] `tmp/release-evidence-check-logs/` is treated as local-only evidence; run `npm run release:evidence:cleanup` before sharing or archiving evidence bundles.

## Evidence log hygiene (pre-share)

- Keep all log files generated by `release:evidence:run` local to `tmp/release-evidence-check-logs/`.
